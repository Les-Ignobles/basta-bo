...
export class RecipeRepository extends BaseRepository<Recipe> {
    ...
    async findPageWithDiets({ search, page, pageSize, noImage, dishType, diets }: { search?: string; page: number; pageSize: number; noImage?: boolean; dishType?: number; diets?: number[] }): Promise<{ data: Recipe[]; total: number }> {
